...
object Converter {
  ...
  private def calculate(basePath: String, filePath: String, content: String, remaining: List[String]): (String, String) = remaining match {
    case line :: tail =>
  ...
  private def buildLineAndDetectFilePath(basePath: String, filePath: String, line: String, content: String): (String, String) = line match {
    case dateRegex(date) => (makeFileName(basePath, filePath, date), content + "\n" + line)
    case urlRegex(url) => (filePath, content + "\n" + makeAlias(url))
    case _ => (filePath, content + "\n" + line)
  }

  private val dateRegex = """^date = (.*)$""".r
  private val urlRegex = """^url = "(.*)"$""".r

  private def makeAlias(url: String): String = s"""aliases = ["/$url/"]"""

  private def makeFileName(basePath: String, filePath: String, date: String): String = {
